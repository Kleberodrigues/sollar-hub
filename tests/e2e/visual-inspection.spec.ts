...
test.describe('Visual Inspection - Sollar Insight Hub', () => {
  ...
  test('Página inicial carrega corretamente', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/Sollar/i);
  ...
  test('Design system - Cores Sollar', async ({ page }) => {
    ...
    const greenElements = await page.locator('[class*="sollar-green"], [class*="bg-sollar-green"]').count();
    const terracottaElements = await page.locator('[class*="terracotta"], [class*="bg-sollar-terracotta"]').count();
  ...
  test('Tipografia - Fontes carregadas', async ({ page }) => {
    ...
    expect(bodyFont.toLowerCase()).toMatch(/inter|system/);
  });
