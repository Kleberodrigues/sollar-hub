...
test.describe('Design System Integration', () => {
  ...
  test('should load custom fonts correctly', async ({ page }) => {
    ...
    // Check if Playfair Display is loaded
    const playfairLoaded = await page.evaluate(() => {
      return document.fonts.check('1em Playfair Display');
    });
    expect(playfairLoaded).toBeTruthy();

    // Check if Lora is loaded
    const loraLoaded = await page.evaluate(() => {
      return document.fonts.check('1em Lora');
    });
    expect(loraLoaded).toBeTruthy();
  });
