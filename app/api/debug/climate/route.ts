...
interface Assessment { id: string; title: string; organization_id: string; start_date: string; }
interface AssessmentDetail { title: string; organization: string; organizationId: string; responses: number; startDate: string; }
...
export async function GET(request: Request) {
  ...
  const { data: orgs } = await supabase
    .from("organizations")
    .select("id, name") as { data: Org[] | null };

  // Find specific org if requested
  let targetOrg: Org | null = null;
  if (orgName && orgs) {
    targetOrg = orgs.find(o => o.name.toLowerCase().includes(orgName.toLowerCase())) || null;
  }

  // Get all climate assessments
  const { data: allAssessments } = await supabase
    .from("assessments")
    .select("id, title, organization_id, start_date")
    .eq("questionnaire_id", CLIMA_QUESTIONNAIRE_ID) as { data: Assessment[] | null };
  ...
  return NextResponse.json({
    ...
    totalResponses: assessmentDetails.reduce((acc, b) => acc + b.responses, 0),
    targetOrg: targetOrg ? {
      id: targetOrg.id,
      name: targetOrg.name,
