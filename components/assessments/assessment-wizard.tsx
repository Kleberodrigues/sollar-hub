...
interface AssessmentWizardProps {
  organizationId: string;
  questionnaires: Array<{ id: string; title: string; questions?: any[] }>;
...
export function AssessmentWizard({
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      // TODO: Disparar webhook diagnostic.activated para n8n
      // TODO: Gerar tokens únicos para participantes

      router.push(`/dashboard/assessments/${data.id}`);
      router.refresh();
    } catch (err: any) {
      console.error('Erro ao ativar assessment:', err);
      setError(err.message || 'Erro ao ativar assessment');
    } finally {
      setLoading(false);
    }
  };
